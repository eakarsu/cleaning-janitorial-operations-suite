const baseUrl = process.env.SMOKE_BASE_URL || 'http://127.0.0.1:5450';
...
const adminCookie = await login('admin@cleaning-janitorial-operations.local', 'admin123');
const managerCookie = await login('manager@cleaning-janitorial-operations.local', 'manager123');
const analystCookie = await login('analyst@cleaning-janitorial-operations.local', 'analyst123');
